...
$sql = "SELECT T.tutorid,T.lastname,T.firstname,T.patronymic,
                                               T.ScientificDegreeID,SD.NAMERU as Scientificdegree_ru,
                                               TC.cafedraid,C.cafedraNameRU,T.BirthDate,T.job_title as positio_ru,
                                               AC.nameru as academicstatus_ru,TP.NameRU AS position_ru,FC.facultyNameRU as faculti_ru,SC.NameRU as sciencefields_ru,
											   TC.rate
                                        ...
                                        LEFT JOIN tutor_cafedra TC ON T.TutorID=TC.tutorID
																				left JOIN tutor_positions TP ON TP.ID=TC.position
                                        ...
                                        where T.deleted=0 AND AC.id=3 AND TC.position=2 AND T.job_title="Профессор" AND TC.type=0 ORDER BY lastname";
...
$sheet->setCellValue("I4",' Позиция в наука');
$sheet->setCellValue("J4",' Факультет');
...
Ставка');
	...
	$sheet->setCellValue("I".$j, $row[$i]['position_ru']);
	$sheet->setCellValue("J".$j, $row[$i][faculti_ru']);
	$sheet->setCellValue("K".$j, $row[$i]['sciencefields_ru']);
	$sheet->setCellValue("L".$j, $row[$i]['BirthDate']);
	$sheet->setCellValue("M".$j, $row[$i]['rate']);
...
<body>
 ...
 <script type="text/javascript"> 
  setTimeout('location.replace("Prepodavatel.php")', 1000); 
</script> 
