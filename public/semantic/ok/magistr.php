...
$sheet->setCellValue("C2", 'Магистр по кафедрам КазНПУ имени Абая на '.date("d.m.Y"));
...
Ставка');
	...
	$j = current_row+$i;
	$sheet->setCellValue("A".$j, $row[$i]['i++']);
	$sheet->setCellValue("B".$j, $row[$i]['lastname']);
	$sheet->setCellValue("C".$j, $row[$i]['firstname']);
	$sheet->setCellValue("D".$j, $row[$i]['patronymic']);
	$sheet->setCellValue("E".$j, $row[$i]['Scientificdegree_ru']);
	$sheet->setCellValue("F".$j, $row[$i]['faculti_ru']);
	$sheet->setCellValue("G".$j, $row[$i]['cafedraNameRU']);
	$sheet->setCellValue("H".$j, $row[$i]['positio_ru']);
	$sheet->setCellValue("I".$j, $row[$i]['academicstatus_ru']);
	$sheet->setCellValue("J".$j, $row[$i]['sciencefields_ru']);
	$sheet->setCellValue("K".$j, $row[$i]['BirthDate']);
	$sheet->setCellValue("L".$j, $row[$i]['rate']);
...
<body>
 ...
 <script type="text/javascript"> 
  setTimeout('location.replace("magistr.php")', 1000); 
</script> 
